 ...
 */
export function formatMessageForDisplay(message, maxLength = 500) {
  if (!message) return '';
  ...
  if (displayMessage.length > maxLength) {
    return displayMessage.substring(0, maxLength) + '...';
  }
...
/**
 * Formats typed data for display in the UI
...
export function formatTypedDataForDisplay(typedData) {
  ...
  try {
    // Create a simplified view of the typed data
    const display = {
